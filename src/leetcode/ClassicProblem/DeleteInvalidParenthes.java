...
// 删除无效括号  https://leetcode-cn.com/problems/remove-invalid-parentheses/
// 第一步确定要删除的左右括号的数量，第二部回溯dfs
// 做的时候第一步就没想出来，觉得没有待删数量的dfs会超时 todo
public class DeleteInvalidParenthes {
    // 用集合存储所有正确的字符串，可避免重复
    private Set<String> set = new HashSet<>();
    public List<String> removeInvalidParentheses(String s) {
        char[] ss = s.toCharArray();
        int open = 0, close = 0;
        // 获取应该去除的左右括号数，非常关键这一步
        for (char c : ss) {
            if (c == '(') open ++;
            else if (c == ')')
                if (open > 0) open --;
